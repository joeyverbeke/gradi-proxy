...
constexpr bool VALVE_ENABLED = false;  // running pump-only rig while valve awaits 6 V rail

// -------------------- PWM (v3.x LEDC pin-scoped)
const uint32_t PUMP_PWM_FREQ = 32000;   // push PWM well above audible range
const uint8_t  PUMP_PWM_RES  = 10;      // 10-bit (0..1023)
const int      DUTY_MAX      = (1 << PUMP_PWM_RES) - 1;

// -------------------- Tunables (MAP-HD-140 @ 3 VDC, 450 mA max)
int precharge_ms = 280;     // time spent easing up to near-peak speed
int puff_ms      = 120;     // high-energy burst window
int guard_ms     = 220;     // let the eyelid recover while holding spin

const int duty_idle_spin   = 180;  // keep the impeller barely spinning
const int duty_precharge   = 720;  // near-peak duty used before each burst
const int duty_peak        = 1000; // slightly below 100% to stay under MAP-HD-140 current ceiling

const int ramp_up_soft_ms  = 140;  // gentle ramp towards precharge duty
const int ramp_up_peak_ms  = 40;   // quick snap into the peak window
const int ramp_down_ms     = 180;  // smooth back to idle spin to hide cogging
const int ramp_to_stop_ms  = 220;  // transition to full stop when sequence ends
...
void startRamp(int fromDuty, int toDuty, uint32_t durationMs) {
  if (fromDuty < 0) fromDuty = 0;
  if (fromDuty > DUTY_MAX) fromDuty = DUTY_MAX;
  if (toDuty < 0) toDuty = 0;
  if (toDuty > DUTY_MAX) toDuty = DUTY_MAX;
  rampFrom  = fromDuty;
  rampTo    = toDuty;
  rampDur   = durationMs;
  rampStart = millis();
...
void startProgrammedSequence() {
  ...
  if (!driverAwake) {
    ...
    driverAwake = true;
    delayMicroseconds(200); // let the H-bridge wake before we spin the pump
  }
  ...
  Serial.print(duty_idle_spin);
  Serial.print('/');
  Serial.print(duty_precharge);
  Serial.print('/');
  Serial.println(duty_peak);
}
...
// -------------------- Valve helpers
// The MAP-HD-140 test rig runs without the FA0520E valve (DRV8833 supply at 3 V),
// but we keep the helpers so the higher-voltage valve path can be reinstated later.
inline void valveOn() {
  if (VALVE_ENABLED) {
    digitalWrite(V_BIN1, HIGH);
    digitalWrite(V_BIN2, LOW);
  }
}
inline void valveOff() {
  if (VALVE_ENABLED) {
    digitalWrite(V_BIN1, LOW);
    digitalWrite(V_BIN2, LOW);
  }
}
...
void enterState(PuffState s, uint32_t now) {
  ...
  switch (puffState) {
    case REST:
      if (sequenceActive) {
        startRamp(currentDuty, duty_idle_spin, ramp_down_ms);
      } else {
        startRamp(currentDuty, 0, ramp_to_stop_ms);
      }
      valveOff();
      break;
    case PRECHARGE:
      startRamp(currentDuty, duty_precharge, ramp_up_soft_ms);
      break;
    case PUFF:
      startRamp(currentDuty, duty_peak, ramp_up_peak_ms);
      valveOn();
      break;
    case RECOVER:
      startRamp(currentDuty, duty_idle_spin, ramp_down_ms);
...
void loop() {
  ...
  if (!sequenceActive && !rampActive && currentDuty == 0 && driverAwake) {
